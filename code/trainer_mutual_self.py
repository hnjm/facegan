...
from miscc.losses import discriminator_loss, generator_loss, generator_vgg_lmk_mutual_self_loss_v2, KL_loss
...
class condGANTrainer(object):
    ...
    def build_models(self):
        ...
        return [
            text_encoder, image_encoder, netG, netsD, epoch, style_loss,
            detector, predictor
        ]
    ...
    def train(self):
        text_encoder, image_encoder, netG, netsD, start_epoch, style_loss, detector, predictor = self.build_models(
        )
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for _ in tqdm(range(len(data_iter))):
                ...
                # else:
                errG_total, G_logs = generator_vgg_lmk_mutual_self_loss_v2(netsD, image_encoder, fake_imgs, real_labels,
                                                                           words_embs, sent_emb, match_labels, cap_lens,
                                                                           class_ids, captions, style_loss, detector, predictor, imgs)
            ...
            for _ in range(1):
                for step, data in enumerate(self.data_loader, 0):
                    ...
                    for j in range(batch_size):
                        ...
                        #     fullpath = '%s_s_%d_g%d.png' % (s_tmp, idx, k)
                        #     im.save(fullpath)
                        k = -1
                        im = fake_imgs[k][j].data.cpu().numpy()
                        im = (im + 1.0) * 127.5
